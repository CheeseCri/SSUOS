...
void timer_handler(struct intr_frame *iframe)
{
	...
	if ((cur_process -> pid != 0) && (!scheduling) && cur_process->state == PROC_RUN) {//sleep 상태 프로세스 1틱 증가시키는 것 방지
		cur_process->time_used++;	
		cur_process->time_slice++;
		if(cur_process->que_level == 1){//큐 레벨이 1일 경우 틱 40 부여
			if(cur_process->time_slice >= TIMER_MAX)
				do_sched_on_return();
		}
		else if(cur_process->que_level == 2){//큐 레벨이 2일 경우 틱 80 부여
			if(cur_process->time_slice >= 80)
				do_sched_on_return();
		}
	}
