...
struct vnode *make_vnode_tree(struct ssufs_superblock *sb, struct vnode *mnt_root)
{
	...
	if(sb != NULL)
		set_vnode(mnt_root, mnt_root, &ssufs_inode_table[2]);

	num_dir = num_direntry((struct ssufs_inode *)mnt_root->info);

	for(int i = 2; i < num_dir; i++){
		ssufs_inode_read((struct ssufs_inode *)mnt_root->info,i *  sizeof(struct dirent), (char*) &buf, sizeof(struct dirent));
		vret = vnode_alloc();
		memset(vret->v_name, 0,LEN_VNODE_NAME); 
		set_vnode(vret, mnt_root,&ssufs_inode_table[buf.d_ino]);
		make_vnode_tree(NULL, vret);
		list_push_back(&mnt_root->childlist, &vret->elem);
	}
...
int ssufs_mkdir(char *dirname){
	...
	//inode 생성 및 초기화
	if((iret = inode_alloc(SSU_DIR_TYPE)) == NULL)
		printk("inode_alloc failed!\n");

	//생성될 inode에 대한 dirent 구조체 입력
	dirent.d_ino = 0;
	dirent.d_type = SSU_DIR_TYPE;
	memset(dirent.d_name, 0, FILENAME_LEN);
	memcpy(dirent.d_name, ".", sizeof("."));
	ssufs_inode_write(iret, 0, (char *)&dirent, sizeof(struct dirent));
	dirent.d_ino = ((struct ssufs_inode*)cur_process->cwd->info)->i_no;
	memset(dirent.d_name, 0, FILENAME_LEN);
	memcpy(dirent.d_name,"..", sizeof(".."));
	ssufs_inode_write(iret,iret->i_size, (char *)&dirent, sizeof(struct dirent));

	//vnode 생성 및 초기화
	if((vret = vnode_alloc()) == NULL)
		printk("vnode_alloc failed!\n");
	vret->type = iret->i_type;
	vret->info = (void *)iret;

	set_vnode(vret, cur_process->cwd, iret);
	memset(vret->v_name, 0, LEN_VNODE_NAME);
	memcpy(vret->v_name, dirname, strlen(dirname));

	dirent.d_ino = iret->i_no;
	memset(dirent.d_name, 0, FILENAME_LEN);
	memcpy(dirent.d_name, dirname, strlen(dirname));

	list_push_back(&cur_process->cwd->childlist, &vret->elem);
	ssufs_inode_write((struct ssufs_inode*)cur_process->cwd->info, ((struct ssufs_inode*)cur_process->cwd->info)->i_size, (char *)&dirent, sizeof(struct dirent));
